...
describe("DecompressInputStream", () => {
	it("decompress without overflowing duplicates bytes count", () => {
		const stream = new DecompressInputStream(new MemoryInputStream(new Buffer([2, 0x02, 4, 0x03, 5, 0x04])), 100);
	...
	it("decompress with overflowing duplicates bytes count", () => {
		const stream = new DecompressInputStream(new MemoryInputStream(new Buffer([255, 0x02, 1, 0x02, 255, 0x03])), 100);
